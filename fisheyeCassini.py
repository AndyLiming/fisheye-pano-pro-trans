...
class Cassini2Fisheye:
  def __init__(self, fish_h, fish_w, ca_h, ca_w, FoV, R=np.identity(3, dtype=np.float32)):
    """
    # fish_h,fish_w: shape of output fish eye images
    # ca_h, ca_w: shape of input erp images
...
class Fisheye2Cassini:
  def __init__(self, fish_h, fish_w, ca_h, ca_w, FoV, R=np.identity(3, dtype=np.float32)):
    self.radius = fish_h // 2
    self.FoV = FoV // 2
    self.FovTh = self.FoV / 180 * np.pi
    ca_x, ca_y = np.meshgrid(range(ca_w), range(ca_h))
    phi = (ca_x).astype(np.float32) / (ca_w - 1) * 2 * np.pi - np.pi
    #phi = ((ca_x).astype(np.float32) - (ca_w - 1) / 2) / (ca_w / 2) * np.pi
    theta = (ca_y).astype(np.float32) / (ca_h - 1) * np.pi
    y = np.cos(theta)
...
if __name__ == '__main__':
  ...
  FoV = 210
  e2f = Cassini2Fisheye(1024, 1024, 512, 1024, FoV, rotate)
  f2e = Fisheye2Cassini(1024, 1024, 512, 1024, FoV, rotate_r)
